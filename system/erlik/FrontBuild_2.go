...
func main() {
	...
	if len(os.Args)<2{
		...
		write("  start loop               - create and start service")
		write("  [X] sistem ortasında veriye ulaşılamayınca tekrar denemiyor")
		write("  stop loop                - delete and stop service")
	...
	if slices.Contains(os.Args, "add") {
		...
		Add( os.Args[2], os.Args[3], duration )
...
`
				...
				if MAPS[name] == "PORT"{
					rules += `
//# `+line+`
__u32 Port_`+name+` = __builtin_bswap16(packet.Port);
...
func Add(name string, value string, duration uint64){
...
func Pin() {
	...
	if err != nil {
		fmt.Printf("%+v\n", err)
		write("ebpf.NewCollection(spec)", spec)
	...
	err = os.Remove("/sys/fs/bpf/map_.bss")
	if err != nil && !os.IsNotExist(err) {
		fmt.Println("remove:", err)
	}

	err = m.Pin("/sys/fs/bpf/map_.bss")
	if err != nil {
			fmt.Printf("BSS PIN ERROR: %T %v\n", err, err)
	} else {
			fmt.Println("BSS PIN OK")
	}
