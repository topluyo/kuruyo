...
$BASE = substr($_SERVER["REQUEST_URI"],strlen("/editor/go")) ."/";
$BASE = explode("?",$BASE)[0];


if($BASE=="" || $BASE=="/"){
  echo "<div padding>/editor/go/web/path/folder</div>";
}
...
if(isset($_GET['new-folder'])){
  mkdir(patify($BASE."/Source/".$_GET['new-folder']));
}
...
if(isset($_GET['new-file'])){
  ...
  $file .= "}";
  file_put_contents(patify($BASE."/Source/".$_GET['new-file'].".go"),$file);
}

if(isset($_GET['save'])){
  file_put_contents(patify($BASE."/Source/".$_POST['file'].".go"),"/*\n".$_POST["comment"]."*/\n".$_POST['code']);
  exit(1);
}
...
$mid .= "  })\n";
  ...
  echo $response;
  if(trim($response)==""){
    echo createService("go1920","/usr/local/go/bin/go run main.go","root",$BASE."Build/");
  }
  exit();
}
...
sort($files);
foreach ($files as $filename) {
  $name = str_replace(patify($BASE."/Source/"),"",$filename);
  $name = str_replace(".go","",$name);
