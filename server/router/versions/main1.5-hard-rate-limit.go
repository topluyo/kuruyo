	...
	for key, level := range server.Levels {
		...
		if len(rates) > 0 {
			...
			for _, rate := range rates {
				...
				limiter[key].Parameters = append(limiter[key].Parameters, param)
			}
		}
	}
...
func RateLimitStress() {
	row("RateLimit Stress")
	// 1. Basit rate limit tanımı
	param := NewRateLimitParameters(1000, 1, 1) // 1000 req/sec, 1 saniye window, 1 sec block
	limit := &Limit{
	...
	fmt.Printf("Total Requests: %d\n", numGoroutines*numRequestsPerGoroutine)
	fmt.Printf("Passed Requests: %d\n", totalPassed)
	fmt.Printf("Elapsed Time: %v\n", elapsed)
	fmt.Printf("Approx RPS: %.0f\n", rps)
}
