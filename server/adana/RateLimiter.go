  ...
  RATE_BITS = server.RateSIZE
	...
	for key, level := range server.Levels {
		...
		if(rate!=""){
			...
			var request,second,status,wait int
			
			if len(RateParams) == 2{
				request = ToNumber(RateParams[0])
				second  = ToNumber(RateParams[1])
				write("   └──", ToString(request)+"r", ToString(second)+"s")
				limit   :=  &Limit{
					Info      : rate,
...
func CheckRateLimiter(ip string, limits []*Limit, status int) int {
	...
	for _, limit := range limits {
		if limit.IsIPBased {
			...
			currentTotals := limit.Totals.Add(1)
			if limit.Request < currentTotals {
				return 503
			}
		}
	}
