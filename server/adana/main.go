...
var server Server 
var TCP_CONNECTIONS atomic.Int64
...
func load(path string){
	...
	if(server.MaxBodySize==0){
		server.MaxBodySize = 1572864 
	}
	if(server.MaxConnection==0){
    server.MaxConnection = 8000
	}
}
	...
	for name , _ := range server.Routes {
		...
		server.Routes[host].proxy = &httputil.ReverseProxy{
			...
			ModifyResponse: func(resp *http.Response) error {
				req := resp.Request
    		ip  := GetIP(req)
				CheckRateLimiter(ip,server.Routes[host].Limits,resp.StatusCode)
				return nil
			},
...
func run(){
    ...
    if _, ok := server.Blocked[ip]; ok {
		...
		if(route.UseLimit){
			//for _, limit := range route.Limits{
				limited := CheckRateLimiter(ip,route.Limits,0)
				if limited!=0 {
					w.WriteHeader(limited)
	...
	srv := &http.Server{
		...
		ConnState: func(c net.Conn, state http.ConnState) {
			switch state {
        case http.StateNew:
					current := TCP_CONNECTIONS.Add(1)
					if current > int64(server.MaxConnection) {
						TCP_CONNECTIONS.Add(-1)
						log.Println("TCP connection limit reached:",current,"/",server.MaxConnection,c.RemoteAddr(),)
						c.Close()
					}
