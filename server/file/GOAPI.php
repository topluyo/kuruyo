...
$BASE = "/var/web/services.topluyo.com/api/Source/";
$TARGET = "/var/web/services.topluyo.com/api/";
...
if(isset($_GET['new-file'])){
  ...
  $file .= "}";
  file_put_contents($BASE.$_GET['new-file'].".go",$file);
}

if(isset($_GET['save'])){
  file_put_contents($BASE.$_POST['file'].".go","/*\n".$_POST["comment"]."*/\n".$_POST['code']);
  exit(1);
}
...
if(isset($_GET['model'])){
  ...
  */
  ...
  if($model_type=="Add"){
    ...
    $response = "type ".$struct_name." struct {\n";
    $response .= "  ".str_pad("ID",24," ")."  ". str_pad("int",10," ")."  ". str_pad("`db:\"name\" json:\"id\"`",24," ")."\n";
    foreach($struct as $s){
      if($s[0]=="ID") continue;
      $response .= "  ".str_pad($s[0],24," ")."  ". str_pad($s[1],10," ")."  ". str_pad($s[2],24," ")."\n";
    }
...
if(isset($_GET['build']) && $_GET['build']==1){
  ...
  $api = "";
  //$output = "";
  $output = file_get_contents($TARGET."Main/main.go");
  //$output .= file_get_contents($TARGET."_main.go");
...
$mid .= "  })\n";
  ...
  file_put_contents($TARGET."Build/api.html", str_replace("//@CODE",$api,file_get_contents($TARGET."Main/api.html")) );
  ...
  file_put_contents($TARGET."Build/"."main.go",$output);

  $lines = explode("\n",$output);

  chdir($TARGET."Build/");
  $cmd = "sudo /usr/local/go/bin/go build main.go 2>&1";
  exec($cmd, $error);
  ...
  echo $response;
  if(trim($response)==""){
    echo createService("go1920","/usr/local/go/bin/go run main.go","root",$TARGET."Build/");
  }
  exit();
}
...
if(isset($_GET["start"])){
  echo createService("go1920","/usr/local/go/bin/go run main.go","root",$TARGET);
}
...
foreach ($files as $filename) {
  ...
  echo "<div using></div>";
  echo "<textarea description style='width:100%;height:7em;margin-bottom:.5em;resize:vertical;'>".$description . "</textarea>\n";

  echo "<div class='editor' name='".$_name."' style='border-radius:6px;'>" . $code ."</div>\n";
